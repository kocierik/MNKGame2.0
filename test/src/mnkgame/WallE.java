...
public class WallE implements MNKPlayer {
  ...
  private AtomicBoolean zobristDone = new AtomicBoolean(false);
  ...
  //Default empty constructor
    ...
    zobristDone.set(false);
    int i = 0, j = 0;
    for (; i < M; i++) {
      for (; j < N; j++) {
        if (j % 15 == 0 && isTimeRunningOut())
          break;
      ...
      new Thread(() -> {
        for (int hh = h; hh < M; hh++) {
          for (int kk = k; kk < N; kk++) {
            zobristTable[hh][kk][0] = random.nextLong();
            zobristTable[hh][kk][1] = random.nextLong();
          }
        }
        zobristDone.set(true);
      }).start();
    } else
      zobristDone.set(true);
  ...
  public MNKCell selectCell(MNKCell[] FC, MNKCell[] MC){
    ...
    while(!isTimeRunningOut() && searchDepth <= maxDepth) {
      if((newCell = negamaxRoot(searchDepth++)) != null){
        bestCell = newCell;
        System.out.println(searchDepth + "diverso");
      }
    }
    ...
    System.out.println("WALLE took " + (start-System.currentTimeMillis()));
    return bestCell;
  }
